...
export class DatabaseService implements OnModuleInit, OnApplicationShutdown {
  ...
  private readonly databaseName = 'remittance_db';
  ...
  private resolveDatabaseUrl(): string {
    ...
    if (!databaseHost || !databaseUser || !databasePassword) {
      throw new Error(
        'remittance 서비스의 DATABASE_HOST / DATABASE_USER / DATABASE_PASSWORD 가(이) 설정되지 않았습니다.',
      );
    }
  ...
  async onModuleInit(): Promise<void> {
    ...
    console.log(`[remittance] PostgreSQL 연결 성공: ${result.rows[0]?.now ?? 'unknown'}`);
  }
